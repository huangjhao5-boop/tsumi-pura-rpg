# Architectural & Implementation Analysis: Milestone 3 Hangar Screen & CRUD (Features 18–21)

**Author:** Explorer 1 (`teamwork_preview_explorer_m3_1`)  
**Target:** Milestone 3 (Features 18, 19, 20, 21)  
**Date:** 2026-09-11  
...
Milestone 3 establishes the **Model Hangar & Showcase Gallery** subsystems. This analysis addresses the core responsibilities assigned to Explorer 1:
- **Feature 18: Model Hangar Screen (`lib/presentation/screens/hangar_screen.dart`)** — List view of all model kits (`KitItem`), status indicators (`[山積]`, `[施工中]`, `[完工]`), and active kit indicators (`★ 當前討伐目標`).
- **Feature 19: Kit CRUD Management** — Add kit modal dialog, Edit kit modal dialog, and Delete kit confirmation dialog with cascade deletion warning.
- **Feature 20: Grade & HP Defaults** — Pre-configured grade presets (`EG`: 300, `HG`: 500, `RG`: 800, `MG`: 1500, `PG`: 5000 HP) based on `GameConstants.gradeHpDefaults`. Selecting a grade automatically populates the HP field.
- **Feature 21: Custom HP Input** — Dedicated toggle/override allowing users to set any positive integer HP (`> 0`), setting `isCustomBoss: true`, with robust input validation.

All designs strictly adhere to the retro 8-bit workbench aesthetic established in `main.dart` and `craft_log_screen.dart` (`#10121A` dark background, `#383A59` pixel borders, Press Start 2P / VT323 typography, and vibrant arcade status chips).

---

## 2. Existing Subsystems & Integration Points

### 2.1 Domain Entities (`KitItem`, `KitStatus`)
`lib/domain/models/kit_item.dart` already provides:
- Canonical status values: `KitStatus.unstarted` (alias `backlog`), `KitStatus.inProgress`, `KitStatus.completed`.
- Helper getters: `kit.isUnstarted`, `kit.isBacklog`, `kit.isInProgress`, `kit.isCompleted`, `kit.isDefeated`, `kit.hpPercentage`.
- Factory `KitItem.create(...)` which automatically derives default HP from `GameConstants.gradeHpDefaults` if `totalHp` is null.
- Entity validation: `kit.validate()` ensuring ID, Title (1..50 chars), HP > 0, 0 <= currentHp <= totalHp.

### 2.2 Data Layer (`KitRepository`)
`lib/data/repositories/kit_repository.dart` provides complete, thread-safe asynchronous operations:
- `getAllKits()`: Loads all kits from local storage, auto-seeding `KitItem.initialSeedKit()` if empty.
- `getActiveKit()`: Retrieves currently targeted Boss kit, falling back to the first non-completed kit.
- `saveKit(KitItem kit)`: Adds or updates kit in persistent storage.
- `deleteKit(String kitId)`: **Crucial:** Automatically performs cascade deletion of all associated `CraftLog` entries via `logRepo.deleteLogsForKit(kitId)`. Automatically picks the next active kit if the deleted kit was active, and re-seeds default kit if all kits are deleted.
- `setActiveKit(String kitId)`: Persists active kit ID to `StorageKeys.activeKitId`.

### 2.3 Shared UI Conventions
Examined `lib/presentation/screens/craft_log_screen.dart` and `lib/main.dart`:
- Palette:
  - Background Canvas: `Color(0xFF10121A)`
  - Card Frame: `Color(0xFF14151F)` with 3px border `Color(0xFF383A59)`
  - Header Banner: `Color(0xFF212234)` with 2px bottom border `Color(0xFF383A59)`
  - Primary Accent Gold: `Color(0xFFFFD54F)`
  - Cyan Accent: `Color(0xFF8BE9FD)`
  - Green Accent: `Color(0xFF50FA7B)`
  - Purple Accent: `Color(0xFFBD93F9)`
  - Orange Accent: `Color(0xFFFFB86C)`
  - Red / Danger Accent: `Color(0xFFFF5252)`
- Font Fallback: `'Press Start 2P'`, `['VT323', 'Courier New', 'monospace']`

---

## 3. Detailed Component Blueprint: `HangarScreen`

### 3.1 Screen Layout & State
File path: `lib/presentation/screens/hangar_screen.dart`

```
HangarScreen (StatefulWidget)
├── SafeArea
│   └── Center
│       └── ConstrainedBox (maxWidth: 540)
│           └── Container (arcade frame, border 3px)
│               ├── Header HUD (_buildHeader)
│               │   ├── Back Button (if canPop)
│               │   ├── Title: "★ MODEL HANGAR ★"
│               │   └── Action: "+ 新增" Button (Key: btn_add_kit)
│               ├── Filter Bar (_buildFilterBar)
│               │   └── SegmentedButton / Retro Filter Chips:
│               │       ["全部 (N)", "山積 (N)", "施工中 (N)", "完工 (N)"]
│               └── Kit List View (_buildKitList)
│                   ├── Empty State (if filtered list is empty)
│                   └── ListView of _KitCard
```

### 3.2 Kit Card Structure (`_KitCard`)
Each card displays:
1. **Header Row**:
   - Grade Badge: `[EG]`, `[HG]`, `[RG]`, `[MG]`, `[PG]` with grade-specific arcade colors.
   - Title: `kit.title` (bold, retro font).
   - Custom Tag: `[自訂]` if `kit.isCustomBoss == true`.
   - Status Badge:
     - `KitStatus.unstarted` -> `[山積]` (grey-blue `Color(0xFF6272A4)`)
     - `KitStatus.inProgress` -> `[施工中]` (amber `Color(0xFFFFB300)`)
     - `KitStatus.completed` -> `[完工]` (bright green `Color(0xFF50FA7B)`)
2. **Active Indicator**:
   - If `kit.id == activeKitId`:
     - Highlight card border with glowing gold `Color(0xFFFFD54F)` (3px).
     - Show prominent ribbon/badge: `★ 當前出擊目標 (ACTIVE BOSS)`.
3. **HP Progress Bar**:
   - Pixelated double-bordered bar:
     - Background: `Colors.black`
     - Foreground fill: dynamic color (Green >50%, Amber >20%, Red <=20%).
   - HP text: `${kit.currentHp} / ${kit.totalHp} HP (${(kit.hpPercentage * 100).toInt()}%)`.
4. **Action Buttons**:
   - **出擊 / 設為目標** (Key: `btn_set_active_${kit.id}`):
     - If active: disabled with label `討伐中 (ACTIVE)`.
     - If inactive: calls `kitRepository.setActiveKit(kit.id)`, updates local state, fires `onKitSelected` callback, shows SnackBar.
     - If completed: disabled or provides "重新挑戰 (Reset & Battle)".
   - **編輯** (Key: `btn_edit_kit_${kit.id}`):
     - Opens `_KitFormDialog` in edit mode.
   - **刪除** (Key: `btn_delete_kit_${kit.id}`):
     - Opens `_DeleteConfirmDialog`.

---

## 4. CRUD Dialog Specifications

### 4.1 Kit Form Dialog (`_KitFormDialog` - Add & Edit)
Can be invoked in two modes: `KitFormMode.create` or `KitFormMode.edit(kit)`.

#### Fields & Interactivity:
1. **Title Input** (`TextFormField`, Key: `input_kit_title`):
   - Controller pre-filled with `kit.title` if editing, or empty.
   - Max length: 50.
   - Validator:
     ```dart
     if (value == null || value.trim().isEmpty) return '請輸入模型名稱';
     if (value.trim().length > 50) return '名稱不可超過 50 字元';
     return null;
     ```
2. **Grade Selection** (Key: `segment_kit_grade`):
   - SegmentedButton or ChoiceChips: `['EG', 'HG', 'RG', 'MG', 'PG']`.
   - Default: `'HG'`.
   - Selection logic:
     - When grade changes, if `_isCustomHp` is `false`:
       - Automatically update `_hpController.text = GameConstants.gradeHpDefaults[newGrade].toString()`.
3. **Custom HP Input & Toggle (Features 20 & 21)**:
   - Checkbox / Switch: `自訂 HP (Custom HP Override)` (Key: `checkbox_custom_hp`).
   - When unchecked:
     - HP field is read-only / disabled or displays the grade default (e.g. `500 HP (HG 預設)`).
     - Changes to Grade immediately overwrite HP.
   - When checked:
     - HP field (`TextFormField`, Key: `input_kit_hp`) is enabled for user entry.
     - `keyboardType: TextInputType.number`
     - `inputFormatters: [FilteringTextInputFormatter.digitsOnly]`
     - Validator:
       ```dart
       if (value == null || value.trim().isEmpty) return '請輸入 HP';
       ...
       ```
4. **Action Buttons**:
   - "取消 (CANCEL)" (Key: `btn_dialog_cancel`): Closes dialog with `Navigator.pop(context)`.
   - "登錄怪獸 (CREATE)" / "儲存變更 (SAVE)" (Key: `btn_dialog_save`):
     - Validates form.
     - In Create mode:
       ```dart
       final newKit = KitItem.create(
         ...
         totalHp: parsedHp,
         isCustomBoss: _isCustomHp || parsedHp != GameConstants.gradeHpDefaults[_selectedGrade],
       );
       await widget.kitRepository.saveKit(newKit);
       if (_setAsActiveImmediately) {
         await widget.kitRepository.setActiveKit(newKit.id);
       }
       ```
     - In Edit mode:
       ```dart
       final updatedTotalHp = parsedHp;
       final updatedCurrentHp = widget.editingKit!.currentHp > updatedTotalHp
           ? updatedTotalHp
           : widget.editingKit!.currentHp;
       final updatedKit = widget.editingKit!.copyWith(
         title: _titleController.text.trim(),
         grade: _selectedGrade,
         totalHp: updatedTotalHp,
         currentHp: _resetHpToFull ? updatedTotalHp : updatedCurrentHp,
         isCustomBoss: _isCustomHp || parsedHp != GameConstants.gradeHpDefaults[_selectedGrade],
       );
       await widget.kitRepository.saveKit(updatedKit);
       ```

### 4.2 Delete Confirmation Dialog (`_DeleteConfirmDialog`)
- Border: Warning Red `Color(0xFFFF5252)` (3px).
- Title: `⚠️ 解體除籍確認 (SCRAP KIT)`
- Message:
  - "確定要將【${kit.title}】從機庫中解體除籍嗎？"
  - "⚠️ 注意：此操作將一併清除該模型的全部施工紀錄 (Craft Logs)，且無法復原！"
  - If `kit.id == activeKitId`: "※ 此模型為當前討伐目標，刪除後將自動切換為下一盒模型。"
- Buttons:
  - "保留 (CANCEL)" (Key: `btn_cancel_delete`)
  - "確認除籍 (DELETE)" (Key: `btn_confirm_delete`, style: Danger Red `Color(0xFFC62828)`)
- Execution:
  ```dart
...
## 5. Complete Production Code Blueprint

Here is the concrete blueprint for `lib/presentation/screens/hangar_screen.dart`:
  ...
  Widget _buildFilterBar() {
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            _buildFilterChip('all', '全部 (${_kits.length})'),
            const SizedBox(width: 6),
            _buildFilterChip('unstarted', '山積 ($unstartedCount)'),
            const SizedBox(width: 6),
            _buildFilterChip('in_progress', '施工中 ($inProgressCount)'),
            const SizedBox(width: 6),
            _buildFilterChip('completed', '完工 ($completedCount)'),
          ],
        ),
      ),
    );
...
### 4.3 `KitCard` Widget Implementation
...
### 4.4 `KitFormDialog` & `DeleteConfirmDialog` Implementation
...
class _KitFormDialogState extends State<KitFormDialog> {
  ...
  void initState() {
    ...
    if (widget.isEditing) {
      ...
      _selectedGrade = _grades.contains(kit.grade) ? kit.grade : 'HG';
  ...
  Future<void> _submit() async {
    ...
    final isCustom = _isCustomHp || parsedHp != GameConstants.gradeHpDefaults[_selectedGrade];
...
Create `test/widget/hangar_screen_test.dart` with comprehensive coverage:
1. **Rendering & Kit Listing (Feature 18)**:
   - Initial loading renders empty state when empty.
   - Initial loading with seeded kits renders kit cards with correct grades, titles, and HP values.
   - Active kit indicator (`★ 當前出擊目標`) appears only on the active kit.
2. **Filter Functionality (Feature 18)**:
   - Tapping "山積" filters only unstarted kits.
   - Tapping "施工中" filters in-progress kits.
   - Tapping "完工" filters completed kits.
   - Tapping "全部" restores all kits.
3. **Setting Active Kit (Feature 18 / 22)**:
   - Tapping "設為目標" on an inactive kit calls `setActiveKit(id)`, updates the UI to show `討伐中`, and highlights the card with gold border.
4. **Adding New Kit & Grade Presets (Feature 19 & 20)**:
   - Tapping "+ 新增" opens `KitFormDialog`.
   - Switching Grade chips (`EG`, `HG`, `RG`, `MG`, `PG`) auto-populates HP field with `300`, `500`, `800`, `1500`, `5000`.
   - Submitting creates new kit in repository and updates Hangar list.
5. **Custom HP Input & Validation (Feature 21)**:
   - Toggling "自訂 HP" enables HP text field.
   - Entering custom value `1200` saves kit with `totalHp: 1200, isCustomBoss: true`.
   - Entering invalid HP (`0`, `-50`, empty) displays validation error.
   - Entering empty title displays validation error.
6. **Editing Kit (Feature 19)**:
   - Tapping "編輯" opens dialog with kit's current details.
   - Modifying title and HP saves and updates list.
7. **Deleting Kit (Feature 19)**:
   - Tapping "刪除" opens warning modal.
   - Confirming deletion calls `deleteKit(id)` and removes the card from the UI.
